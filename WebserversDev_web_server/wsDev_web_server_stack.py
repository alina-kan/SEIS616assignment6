...
class wsDevWebServerStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, engineering_vpc: ec2.Vpc, **kwargs) -> None:
        ...
        web_sg = ec2.SecurityGroup(self, "WebServerSG", vpc=engineering_vpc, description="Allow HTTP traffic to web servers")
        web_sg.add_ingress_rule(ec2.Peer.any_ipv4(), ec2.Port.tcp(80), "Allow HTTP from anywhere")

        # Create EC2 instances in public subnets
        for i, subnet in enumerate(engineering_vpc.public_subnets):
        ...
        rds_sg = ec2.SecurityGroup(self, "RDSSG", vpc=engineering_vpc, description="Allow MySQL traffic from web servers")
        ...
        cdk_lab_web_instance = ec2.Instance(self, "cdk_lab_web_instance", vpc=engineering_vpc,
                                            instance_type=ec2.InstanceType("t2.micro"),
                                            machine_image=ec2.AmazonLinuxImage(generation=ec2.AmazonLinuxGeneration.AMAZON_LINUX_2),
                                            role=InstanceRole)
